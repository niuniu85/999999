...
router.post("/clientListDistribution", function (req, res, next) {
  ...
  console.log(typeof userName[2]);
  if (typeof userName[2] !== undefined) {
              ...
              dbo
                ...
                .updateMany(whereStr, updateStr, function (err, result1) {
                  ...
                  if (result1.matchedCount > 0) {
                    ...
                    dbo.collection("clientListDistributionLog").insertOne(logStr, function (err, result2) {
                      if (err) throw err;
                      console.log("分配记录成功");
                      db.close();
                    });
...
router.post("/clientListDistributionTo", function (req, res, next) {
  let selectedRowKeys = req.body.selectedRowKeys;
  if (typeof selectedRowKeys !== undefined) {
    ...
    try {
      MongoClient.connect(url, function (err, db) {
        ...
        dbo
          ...
          .find({ _id: { $in: selectedRowKeys } })
          .toArray(function (err, result) {
            // 返回集合中所有数据
            if (err) throw err;
            if (typeof result !== undefined) {
              console.log("公海转移至私有：" + JSON.stringify(result));
              dbo.collection("client").insertMany(result, function (err, result1) {
                try {
                  if (err) throw err;
                  if (result1) {
                    dbo.collection("openclient").deleteMany({ _id: { $in: selectedRowKeys } }, function (err, result2) {
                      if (err) throw err;
                      if (result2) {
                        res.send({
                          success: true,
